-- Prevent stale devices from replacing marking codes that were already saved.

alter type public.sync_operation_type add value if not exists 'set_marking_codes';

create or replace function app_private.normalize_marking_codes(value jsonb)
returns jsonb
language sql
immutable
set search_path = public
as $$
  select coalesce(jsonb_object_agg(item_key, codes), '{}'::jsonb)
  from (
    select
      item.key as item_key,
      jsonb_agg(distinct code.value order by code.value) as codes
    from jsonb_each(coalesce(value, '{}'::jsonb)) as item
  ...
  where jsonb_array_length(codes) > 0;
$$;

create or replace function app_private.marking_counts(marking_codes jsonb)
returns jsonb
language sql
immutable
set search_path = public
as $$
  select coalesce(jsonb_object_agg(item.key, jsonb_array_length(item.value)), '{}'::jsonb)
  from jsonb_each(app_private.normalize_marking_codes(marking_codes)) as item;
$$;

revoke all on function app_private.normalize_marking_codes(jsonb) from public;
revoke all on function app_private.marking_counts(jsonb) from public;
