alter table public.orders
  add column if not exists marking_codes jsonb not null default '{}'::jsonb,
  add column if not exists fiscal_receipt jsonb not null default '{"status":"not_required"}'::jsonb;

alter table public.sync_operations
  alter column operation_id type text;

create index if not exists orders_fiscal_receipt_status_idx
on public.orders ((fiscal_receipt->>'status'));
